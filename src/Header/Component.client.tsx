...
export const HeaderClient: React.FC<HeaderClientProps> = ({ data }) => {
  ...
  // Prevent body scroll when menu is open (especially important for iOS)
  useEffect(() => {
    if (isMenuOpen) {
      // Save current scroll position
      const scrollY = window.scrollY;
      // Lock body and html scroll (html needed for iOS Safari)
  ...
  return (
    ...
    >
      ...
      >
        {({ open }) => {
          // Sync menu open state (using ref to avoid hook rules violation)
          ...
          return (
            <>
              ...
              <div
                className={cn(
                  'ease-out duration-300 overflow-hidden shadow-lg dark:shadow-[inset_0px_-1px_1px_#132f4c] fixed h-[100vh] top-20 w-full bg-white block overscroll-none touch-none',
                  { 'left-0': open },
                  { 'left-[100%]': !open },
                )}
